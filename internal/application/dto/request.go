...
type CreateMessageCampaignRequest struct {
	...
	NotificationTypes *uint8     `json:"notification_types"                           binding:"required,min=1,max=7"                                                                 example:"3"                                                                                                          validate:"required,min=1,max=7"`                                  // 推送類型位元遮罩：1=站內信，2=App推播，3=兩者皆有，可組合至7，必填
	Target            string     `json:"target"                                       binding:"required,oneof=high_activity low_activity not_activity player level tag all"          example:"player"                       enums:"high_activity,low_activity,not_activity,player,level,tag,all"          validate:"required"`                                              // 目标用户
	...
	Status            string     `json:"status"                                       binding:"required,oneof=draft scheduled"                                                       example:"scheduled"                                                                                                  validate:"required"`                                              // 活动状态
...
type UpdateMessageCampaignRequest struct {
	...
	Content           string     `json:"content"                                           binding:"required"                                                                             example:"恭喜您成功注册成为我们的会员！"                                                                                    validate:"required"`                                              // 消息内容，必填
	AppContent        *string    `json:"app_content,omitempty"                                                                                                                            example:"恭喜您成功注册！"                                                                                           validate:"omitempty,max=255"`                                     // App推播內容，可選，最大255個字符
	NotificationTypes *uint8     `json:"notification_types,omitempty"                      binding:"omitempty,min=1,max=7"                                                                example:"3"                                                                                                  validate:"omitempty,min=1,max=7"`                                 // 推送類型位元遮罩：1=站內信，2=App推播，4=其他，可選
...
type SendAutoNotificationRequest struct {
	...
	Category       string `json:"category"         binding:"required,oneof=member bonus"                                         example:"member"                                      validate:"required,oneof=member bonus"    enums:"member,bonus"`                                         // 訊息類別：member=會員訊息，bonus=紅利訊息
	Item           string `json:"item"             binding:"required,oneof=registration identity_verification bank_card mission" example:"registration"                                validate:"required"                       enums:"registration,identity_verification,bank_card,mission"` // 訊息項目：registration=註冊，identity_verification=身分驗證，bank_card=銀行卡，mission=任務
	TriggerType    string `json:"trigger_type"     binding:"required,oneof=success failure"                                      example:"success"                                     validate:"required,oneof=success failure" enums:"success,failure"`                                      // 觸發類型：success=成功，failure=失敗
}
