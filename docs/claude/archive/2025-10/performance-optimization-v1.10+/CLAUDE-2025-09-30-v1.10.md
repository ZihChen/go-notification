...
### 核心改進
1. **關聯表正規化**：從 JSON 解析改為直接 SQL JOIN 查詢
2. **統一ID處理**：所有target類型統一使用 `uint64` 數值ID
3. **批量查詢優化**：消除 N+1 查詢問題
4. **雙寫機制**：確保向後兼容性的同時提供性能提升
5. **智能路由**：根據可用數據自動選擇最優查詢路徑
...
| 資料庫查詢次數 | N+1次 | 1次 | ~95% |
| 記憶體使用 | 所有campaigns | 僅ID列表 | ~90% |
| Player驗證查詢 | 重複查詢 | 單次批量 | ~50% |

### Phase 1: Campaign Targets 關聯表正規化 ✅

#### 1.1 新增關聯表結構 ✅
```sql
-- Campaign Target 關聯表 (優化版 - 儲存數值ID)
CREATE TABLE `campaign_targets` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `campaign_id` bigint unsigned NOT NULL,
  `target_type` varchar(20) NOT NULL COMMENT 'player/level/tag/all',
  `target_id` bigint unsigned NULL COMMENT '目標數值ID：player_id/level_id/tag_id，all類型為NULL',
...
type CampaignTarget struct {
    ...
    TargetID   *uint64    `gorm:"index:idx_target_type_id"` // 數值ID，支援NULL
...
#### 2.5.2 批量查詢優化 ✅
```go
// validatePlayerAccounts 改為批量查詢並返回IDs
...
### Week 1-2: 基礎建設 ✅ (已完成)
- [x] 設計並創建 `campaign_targets` 表
- [x] 實作 Entity 和 Repository 介面
- [x] 撰寫資料遷移腳本

### Week 3: 雙寫實作 ✅ (已完成)
- [x] 修改 Campaign CRUD 操作支援雙寫
- [x] 實作新的查詢邏輯
- [x] 添加特性開關控制新舊邏輯
- [x] 統一ID處理機制
- [x] 批量查詢優化
